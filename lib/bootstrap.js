  ...
  return got(`https://api.npms.io/v2/search?q=${themePrefix}`, { json: true })
    .then(({ body }) => {
      const themes = body.results
        ...
        .filter(({ name }) => name.includes(themePrefix))
    ...
    .catch(() => {
      spinner.text = chalk.red(
        'There was a problem fetching theme suggestions. Using default theme...'
      );
      spinner.fail();
      return [defaultTheme];
    });
...
const createInstallSpinner = promise => {
  const spinner = ora(
    chalk.dim('Installing dependencies. This may take a while...')
  ).start();
  return promise
    .then(() => {
...
const bootstrapDirectory = dir => {
  const melodramaPkgData = {
    scripts: {
      start: 'melodrama-scripts start index.js',
      build: 'melodrama-scripts build index.js',
    },
    private: true,
    readme: 'README.md',
  };
...
const prepareDependencies = ({ syntax, theme }) => {
  ...
  if (syntax) {
    dependencies.push('prismjs');
  }
  if (theme && theme !== defaultTheme) {
    dependencies.push(`${themePrefix}${theme}`);
  }
  return dependencies;
};
